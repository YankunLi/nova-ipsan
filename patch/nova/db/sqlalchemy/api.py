...
#change at 20150415add ipsan api
#author : liyankun
...
def instance_get(context, instance_id, columns_to_join=None):
    try:
        ...
        LOG.debug(_("instance_by_id:%(instance)s"),{'instance':result})
...
@require_admin_context
def ipsan_get_by_instance_id(context, instance_id):
    session = get_session()
    with session.begin():
        query = model_query(context, models.Ipsan, session = session,
                            read_deleted = 'yes').filter_by(instance_uuid=instance_id).all()
                        #    first()
#    result = query.first()
...
@require_admin_context
def ipsan_get_by_ipsan_id(context, ipsan_id):
    session = get_session()
    with session.begin():
   #     query = model_query(context, models.Instance, session = session).filter_by(id=ipsan_id).first()
        query = model_query(context, models.Ipsan, session = session,
                            read_deleted = 'yes').filter_by(id=ipsan_id).\
                            first()
    return query


@require_admin_context
def ipsan_get_all(context):
    session = get_session()
    with session.begin():
        ipsan_list = model_query(context, models.Ipsan, session = session,
...
@require_admin_context
def attach_ipsan_to_instance(context, ipsan_id, values):
    session = get_session()   
    
    with session.begin():
        ipsan = model_query(context, models.Ipsan, session=session, read_deleted = 'yes').\
        filter_by(id = ipsan_id).\
        first()
        
        if not ipsan or ipsan.status in ['attached', 'unavailable']:
            return 
        else:
...
@require_admin_context
def detach_ipsan_from_instance(context, ipsan_id, values):
    session = get_session()   
    ...
    with session.begin():
        ...
        if not ipsan or ipsan.status in ['unavailable']:
            return 
        else:
            values['status'] = 'detach'
        ipsan.update(values)
...
@require_admin_context
def ipsan_update(context, ipsan_id, values):
    session = get_session()
...
@require_admin_context
def ipsan_create(context, values):
#     uuid = values.get('uuid', None)
#     if uuid is None:
#         uuid = uuidutils.generate_uuid()
#         values['uuid'] = uuid
...
#             raise exception.InstanceGroupIdExists(group_uuid=uuid)
    ...
    with session.begin():
        ...
        try:
            ...
            raise Exception("dffffffffffffffffffffffff")

    return ipsan

@require_admin_context
def ipsan_delete(context, ipsan_id):
    """Delete a ipsan record."""
    session = get_session()
    with session.begin():
        result = model_query(context, models.Ipsan, session=session, read_deleted = 'yes').\
                 filter_by(id=ipsan_id).\
                 soft_delete()
