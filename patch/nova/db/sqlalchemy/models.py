...
"""
#change at 20150415 add Ipsan model
#author : liyankun
...
class Ipsan(BASE, NovaBase):
    """Represents a ipsan device"""
    __tablename__ = 'ipsans'
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    target = Column(String(255), nullable=False)
    ip = Column(String(36), nullable=False)
    port = Column(String(36), nullable=False)
    auth_method = Column(String(36))
    username = Column(String(36))
    password = Column(String(36))
    size = Column(String(36), nullable=False)
    status = Column(String(36), nullable=False)
    instance_uuid = Column(Integer, ForeignKey('instances.id'))
    instance = relationship("Instance", backref="ipsans")
    device_name = Column(String(36))

#class InstanceIpsan(BASE, NovaBase):
 #   """Represents a user-provided ipsan key/value pair for an instance."""
 #   __tablename__ = 'instance_ipsans'
 #   id = Column(Integer, primary_key=True,autoincrement=True)
 #   ipsan_id = Column(Integer, ForeignKey('ipsans.id'))
 #   instance_uuid = Column(String(36), ForeignKey('instances.uuid'))
 #   instance = relationship(Instance, backref="ipsan",
  #                          foreign_keys=instance_uuid,
 #                           primaryjoin='and_('
 #                               'InstanceIpsan.instance_uuid == '
  #                                   'Instance.uuid,'
  #                              'InstanceIpsan.deleted == 0)')
