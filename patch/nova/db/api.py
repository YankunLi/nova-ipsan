...
def ipsan_get_by_ipsan_id(context, ipsan_id):
    """get a ipsan or raise if does not exist."""
    return IMPL.ipsan_get_by_ipsan_id(context, ipsan_id)

def ipsan_get_by_instance_id(context, instance_id):
    """get a ipsan or raise if does not exist."""
    return IMPL.ipsan_get_by_instance_id(context, instance_id)

def ipsan_get_all(context):
    """get a list of ipsan"""
    return IMPL.ipsan_get_all(context)

def ipsan_create(context, values):
    """create a ipsan record"""
    return IMPL.ipsan_create(context, values)

def ipsan_update(context, ipsan_id, values):
    """upgrade a ipsan entry that is exist"""
    return IMPL.ipsan_update(context, ipsan_id, values)

def ipsan_delete(context, ipsan_id):
    """delete a ipsan entry"""
    IMPL.ipsan_delete(context, ipsan_id)
# Copyright (c) 2011 X.commerce, a business unit of eBay Inc.
# Copyright 2010 United States Government as represented by the
# Administrator of the National Aeronautics and Space Administration.
# All Rights Reserved.
