...
MIN_LIBVIRT_LIVESNAPSHOT_VERSION = (0, 10, 2)
MIN_QEMU_LIVESNAPSHOT_VERSION = (0, 12, 1)
...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def get_host_device(self, iscsi_properties):
        return ("/dev/disk/by-path/ip-%s-iscsi-%s-lun-%s" % 
                (iscsi_properties['target_portal'],
                 iscsi_properties['target_iqn'],
                 iscsi_properties.get('target_lun', 0)))
    def run_iscsiadm(self, ipsan_info, command):
        utils.execute('iscsiadm', '-m', 'node',
                      '-T', ipsan_info['target_iqn'],
                      '-p', ipsan_info['target_portal'],
                      *command, run_as_root=True)
    def iscsi_auth(self, ipsan_info, command):
        utils.execute('iscsiadm', '-m', 'node',
                      '-T', ipsan_info['target_iqn'],
                      '-p', ipsan_info['target_portal'],
                      '--op', 'update', '-n', 
			*command, run_as_root=True)
    def ipsan_to_xml(self, block_device_mapping, pos, id):
        conf = vconfig.LibvirtConfigGuestDisk()
        conf.driver_name = "qemu"
        connection_info = block_device_mapping[pos][id]
        LOG.debug(_('connection_vol:%(block_device_mapping)s'),{'block_device_mapping':connection_info})
        conf.source_device = "disk"
        conf.driver_format = "raw"
        conf.driver_cache = "none"
        conf.target_dev = connection_info['data']['dev']
        conf.target_bus = "virtio"
        conf.source_type = "block"
        iscsi_properties = connection_info['data']
        host_device = self.get_host_device(iscsi_properties)
        conf.source_path = host_device
        iscsi_command = ('--op', 'new')
        self.run_iscsiadm(iscsi_properties, iscsi_command)
        self.iscsi_auth(iscsi_properties, ('node.session.auth.authmethod', '-v', iscsi_properties['auth_method']))
        self.iscsi_auth(iscsi_properties, ('node.session.auth.username', '-v', iscsi_properties['auth_username']))
        self.iscsi_auth(iscsi_properties, ('node.session.auth.password', '-v', iscsi_properties['auth_password']))
        self.run_iscsiadm(iscsi_properties, ('--login',))
        return conf
    def ipsan_config(self,context, block_device_mapping, metadata):
        connection_info_all = {}
        ipsan_id = metadata['ipsan'].split(',')
        for id in ipsan_id:
            ipsan_info  = db_api.ipsan_get_by_ipsan_id(context, id)
            LOG.debug(_('ipsan_id:%(id)s, ipsan_info:%(ipsan_info)s'),{'id':id, 'ipsan_info': ipsan_info})
            if ipsan_info:
                target_portal = ':'.join([ipsan_info.ip,ipsan_info.port])
                connection_info = {'driver_volume_type': 'iscsi', 'serial': None,
                       'data': {'access_mode': 'rw', 'target_discovered': False, 'dev': ipsan_info.device_name,
                       'encrypted': False, 'qos_specs': None, 'target_iqn': ipsan_info.target,
                       'target_portal': target_portal, 'volume_id': None, 'target_lun': 1,
                       'auth_password': ipsan_info.password, 'auth_username': ipsan_info.username, 'auth_method': ipsan_info.auth_method}}
                connection_info_all[id] = connection_info
                block_device_mapping.append(connection_info_all)
	return block_device_mapping

    def get_guest_storage_config(self, context, instance, image_meta,
                                 disk_info, 
        ...
        block_device_mapping = driver.block_device_info_get_mapping(
            block_device_info)
        # From the db get the ipsan data
        # Get the instance metadata for generating xml 
        metadata = instance.get('metadata')
        if 'ipsan' in metadata:
	    block_device_mapping = self.ipsan_config(context, block_device_mapping, metadata)
        ...
        if CONF.libvirt.virt_type == "lxc":
		    ...
		    pos = 0
		    for id in ipsan_id:
		        conf = self.ipsan_to_xml(block_device_mapping, pos, id)
	                devices.append(conf)
			pos = pos+1
                else:                
                    for vol in block_device_mapping:
    ...
    def get_guest_config(self, context, instance, network_info, image_meta, 
        ...
        for cfg in self.get_guest_storage_config(context,
						 instance,
                                                 image_meta,
                                                 disk_info,
                                                 rescue,
    ...
    def to_xml(self, context, instance, network_info, disk_info,
        ...
        LOG.debug(_('Start to_xml '
                  ...
                  {'network_info': network_info_str, 'disk_info': disk_info,
                   ...
                   'block_device_info': block_device_info},
                  instance=instance)
        conf = self.get_guest_config(context, instance, network_info, image_meta,
    ...
    def finish_migration(self, context, migration, instance, disk_info,
        ...
        xml = self.to_xml(context, instance, network_info, disk_info,
                          ...
                          write_to_disk=True)
        self._create_domain_and_network(context, xml, instance, network_info,
                                        block_device_info, power_on, vifs_already_plugged=True)
        if power_on:
            timer = loopingcall.FixedIntervalLoopingCall(
