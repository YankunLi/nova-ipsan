...
class API(base.Base):
    ...
    def attach_ipsan_to_instance(self, context, base_options, instances):
        device_name = ['sdg','sdf','sde','sdd','sdc','sdb']
        ipsan_id = base_options['metadata']['ipsan'].split(',')
        for id in ipsan_id:
            values = {}
            values['instance_uuid']=  instances[0]['id']
            values['status'] = 'attach'
            values['device_name'] = device_name.pop()
            db_api.attach_ipsan_to_instance(context, id, values)
    def check_ipsan_valid(self, context, base_options):
	ipsan_id = base_options['metadata']['ipsan'].split(',')
	for id in ipsan_id:
	    ...
	    if ipsan_info != None:
		    ...
		    utils.execute('iscsiadm', '-m', 'node',
                      '-T', ipsan_info.target,
                      '-p', target_portal,
                      '--login', run_as_root=True)
	        except Exception as exc:
	            LOG.error(_("Try to login ip san %(ipsan)s "
				"The result is %(reason)s"),
				{'ipsan': ipsan_info.target,
				 'reason': exc})
	    else:
                LOG.error(_("The id of ipsan: %(id)s does not exist!"),{'id':id})
	...
	if 'ipsan' in base_options['metadata']:
	    self.check_ipsan_valid(context, base_options)	
	    self.attach_ipsan_to_instance(context, base_options, instances)
    ...
    def _delete(self, context, instance, delete_type, cb, **instance_attrs):
        ...
        for bd in bdms.objects:
            if bd.source_type == "volume" and bd.destination_type == "volume" and bd.boot_index != 0:
                raise exception.CannotDeleteInstanceHaveVolumeMapping()
        reservations = None
    ...
    def detach_ipsan_from_instance(self, context, instance):
        lists = instance.ipsans
        values = {'instance_uuid':instance['id'], 'device_name':''}
        for lt in lists:
            dirct = {}
            for item in lt:
                key = item['key']
                value = item['value']
                dirct.update({key:value})
            id = dirct.get('id')
            db_api.detach_ipsan_from_instance(context, id, values)

    def delete(self, context, instance):
        """Terminate an instance."""
        if  instance.ipsans:
	    self.detach_ipsan_from_instance(context, instance)
    ...
    def get(self, context, instance_id, want_objects=False,
        ...
        if not expected_attrs:
            expected_attrs = []
        expected_attrs.extend(['metadata', 'system_metadata', 'ipsans',
